...
export class allMigration1634379140718 implements MigrationInterface {
    name = 'allMigration1634379140718'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`kru\`.\`user\` (\`id\` int NOT NULL AUTO_INCREMENT, \`uuId\` char(36) NOT NULL, \`generateId\` varchar(255) NOT NULL, \`email\` varchar(255) NOT NULL, \`fullname\` varchar(255) NOT NULL, \`profile\` varchar(255) NOT NULL, \`score\` int NOT NULL, \`updateAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`majorId\` int NOT NULL, UNIQUE INDEX \`IDX_5c86b7794f9882001aadeab6e4\` (\`uuId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`kru\`.\`forum\` (\`id\` int NOT NULL AUTO_INCREMENT, \`uuId\` char(36) NOT NULL, \`topic\` text NOT NULL, \`userId\` int NOT NULL, \`updateAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_4406746e259405498b0bbba1c5\` (\`uuId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`kru\`.\`detail\` (\`id\` int NOT NULL AUTO_INCREMENT, \`uuId\` char(36) NOT NULL, \`answer\` text NOT NULL, \`forumId\` int NOT NULL, \`userId\` int NOT NULL, \`updateAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_634ab4e34f6111bb4db4c403d4\` (\`uuId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`kru\`.\`detail\` ADD CONSTRAINT \`FK_bfed71929534e214a2d779c3a16\` FOREIGN KEY (\`forumId\`) REFERENCES \`kru\`.\`forum\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`kru\`.\`detail\` DROP FOREIGN KEY \`FK_bfed71929534e214a2d779c3a16\``);
        ...
        await queryRunner.query(`DROP INDEX \`IDX_634ab4e34f6111bb4db4c403d4\` ON \`kru\`.\`detail\``);
        await queryRunner.query(`DROP TABLE \`kru\`.\`detail\``);
